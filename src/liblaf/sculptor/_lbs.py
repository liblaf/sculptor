"""Modified from <https://github.com/sculptor2022/sculptor/blob/main/model/lbs.py>."""
# -*- coding: utf-8 -*-
...
def Trans_WG(rot_mats, joints, parents, jaw_offset, lbs_weights, batch_size, dtype):
    J_transformed, A = batch_rigid_transform(
        rot_mats, joints, parents, jaw_offset, dtype=dtype
    )
    # 5. Do skinning:
    # W is N x V x (J + 1)
    ...
    T = torch.matmul(W, A.view(batch_size, num_joints, 16).float()).view(
        batch_size, -1, 4, 4
    )
...
def Homo_Coord(batch_mesh):
    ...
    homogen_coord = torch.ones(
        [batch_size, batch_mesh.shape[1], 1], dtype=torch.float32
    ).cuda()
...
def LinearBlendSkinning(V_Refine, TransMtx, batch_size, dtype, device):
    homogen_coord = torch.ones(
        [batch_size, V_Refine.shape[1], 1], dtype=dtype, device=device
    )
    V_Refine_homo = torch.cat([V_Refine, homogen_coord], dim=2)

    v_homo = torch.matmul(
        TransMtx.float(), torch.unsqueeze(V_Refine_homo.float(), dim=-1)
    )
...
def transform_mat(R: Tensor, t: Tensor) -> Tensor:
    ...
    """
    return torch.cat(
        [F.pad(R, [0, 0, 0, 1]), F.pad(t.float(), [0, 0, 0, 1], value=1)], dim=2
    ).float()
