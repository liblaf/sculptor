"""Modified from <https://github.com/sculptor2022/sculptor/blob/main/model/sculptor.py>."""
...
from ._data import load_paradict
from ._lbs import LinearBlendSkinning, PoseBlendShape, ShapeBlendShape, Trans_WG
from ._utils import to_tensor
...
class SCULPTOR_layer(nn.Module):
    def __init__(self) -> None:
        ...
        self.template_skull, self.template_face = (
            sculptor_para["template_skull"],
            sculptor_para["template_face"],
        )

        self.skull_shape, self.face_shape = (
            sculptor_para["skull_shape"],
            sculptor_para["face_shape"],
        )

        self.skull_div, self.face_div = (
            sculptor_para["skull_div"],
            sculptor_para["face_div"],
        )
        self.register_buffer(
            "template",
            to_tensor(
                np.vstack(
                    [sculptor_para["template_skull"], sculptor_para["template_face"]]
                )
            ),
        )

        face_shape_dir = np.vstack(
            [sculptor_para["face_div"], sculptor_para["face_shape"]]
        )
        skull_shape_dir = np.vstack(
            [sculptor_para["skull_div"], sculptor_para["skull_shape"]]
        )
    ...
    def forward(self, beta_s, pose_theta, jaw_offset):
        ...
        pose_offset, rot_mats = PoseBlendShape(
            Full_pose, self.pose_dir, batch_size, device, dtype
        )
        T, _ = Trans_WG(
            rot_mats,
            joints,
